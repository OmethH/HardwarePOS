...
func (h *Handler) RegisterRoutes(rg *gin.RouterGroup) {
	...
	write.POST("/suppliers", h.create)
	write.PUT("/suppliers/:id", h.update)
	write.DELETE("/suppliers/:id", h.delete)
}

func (h *Handler) list(c *gin.Context) {
	items, err := h.svc.List(c.Query("search"))
	if err != nil {
		httpresp.Err(c, 500, "failed to load suppliers")
		return
	}
...
func (h *Handler) get(c *gin.Context) {
	...
	if err != nil {
		httpresp.Err(c, 404, "supplier not found")
		return
	}
...
func (h *Handler) create(c *gin.Context) {
	...
	if err != nil {
		httpresp.Err(c, 400, "could not create supplier")
		return
	}
...
func (h *Handler) delete(c *gin.Context) {
	...
	if err := h.svc.Delete(uint(id)); err != nil {
		httpresp.Err(c, 400, "could not delete supplier")
		return
	}
