...
func (h *Handler) RegisterRoutes(rg *gin.RouterGroup) {
	...
	group.GET("/purchases", h.list)
	group.GET("/purchases/:id", h.get)
	group.POST("/purchases", h.create)
}
...
func (h *Handler) list(c *gin.Context) {
	...
	if err != nil {
		httpresp.Err(c, 500, "failed to load purchases")
		return
	}
...
func (h *Handler) get(c *gin.Context) {
	...
	if err != nil {
		httpresp.Err(c, 404, "purchase not found")
		return
	}
...
func (h *Handler) create(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		httpresp.Err(c, 400, "supplier_id and at least one item are required")
		return
	}
